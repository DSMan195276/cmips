...
/*
 * This is a structure for holding a rbtree (By hold, it hold the location of
...
/*
 * This crazy macro implements a loop through nodes using pre/in/post order
 * functions, defined by the 'type'.
 *
 * This below code acomplishes a basic for-loop through every node. It's
 * complicated to make break; and continue; work as expected, internally
 * declare the rb_trav_state so it doesn't have to be supplied, and make the
 * entire thing a prefix-statement, so it can be used as expected of a normal
 * for statement, allowing a single-line loop or a block of code via { }.
 *
 * The basic idea is we declare _rb_state, run the initalize code, and jump to
 * our check which either jumps to our loop body code or jump to the loop exit.
