...
static void buf_blk_free(struct buf_blk *blk)
{
    if (empty_blocks_byte_count < BLK_EMPTY_BYTE_MAX) {
        blk->next_blk = empty_blocks_head;
        empty_blocks_head = blk->next_blk;
        empty_blocks_byte_count += blk->allocsize;
    } else {
        free(blk);
    }
}
...
static struct buf_blk *new_buf_block(struct buf_fd *buf)
{
    ...
    if (empty_blocks_head == NULL) {
        ...
        new = empty_blocks_head;
        empty_blocks_head = empty_blocks_head->next_blk;
        empty_blocks_byte_count -= new->allocsize;
        new->next_blk = NULL;
        new->offset = 0;
        new->size = 0;
    }
